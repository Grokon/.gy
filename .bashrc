...
# Get cpu count
case "$LP_OS" in
    Linux)   NCPU=$( nproc 2>/dev/null || grep -c '^[Pp]rocessor' /proc/cpuinfo ) ;;
    FreeBSD|Darwin|OpenBSD) NCPU=$( sysctl -n hw.ncpu ) ;;
    SunOS)   NCPU=$( kstat -m cpu_info | grep -c "module: cpu_info" ) ;;
esac
